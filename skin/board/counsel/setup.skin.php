...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
		...
		<td>
			<label><input type="checkbox" name="boset[email]" value="1"<?php echo get_checked('1', $boset['email']);?>> 이메일</label>
			&nbsp;
			<label><input type="checkbox" name="boset[homepage]" value="1"<?php echo get_checked('1', $boset['homepage']);?>> 홈페이지</label>
			&nbsp;
			<label><input type="checkbox" name="boset[tel]" value="1"<?php echo get_checked('1', $boset['tel']);?>> 전화번호</label>
			&nbsp;
			<label><input type="checkbox" name="boset[hp]" value="1"<?php echo get_checked('1', $boset['hp']);?>> 모바일</label>
			&nbsp;
			<label><input type="checkbox" name="boset[comname]" value="1"<?php echo get_checked('1', $boset['comname']);?>> 업체명</label>
			&nbsp;
			<label><input type="checkbox" name="boset[addr]" value="1"<?php echo get_checked('1', $boset['addr']);?>> 주소</label>
		</td>
	</tr>
	<tr>
		<td align="center">필수설정</td>
		<td>
			<label><input type="checkbox" name="boset[req_email]" value="1"<?php echo get_checked('1', $boset['req_email']);?>> 이메일</label>
			&nbsp;
			<label><input type="checkbox" name="boset[req_tel]" value="1"<?php echo get_checked('1', $boset['req_tel']);?>> 전화번호</label>
			&nbsp;
			<label><input type="checkbox" name="boset[req_hp]" value="1"<?php echo get_checked('1', $boset['req_hp']);?>> 모바일</label>
			&nbsp;
			<label><input type="checkbox" name="boset[req_comname]" value="1"<?php echo get_checked('1', $boset['req_comname']);?>> 업체명</label>
			&nbsp;
			<label><input type="checkbox" name="boset[req_addr]" value="1"<?php echo get_checked('1', $boset['req_addr']);?>> 주소</label>
		</td>
	</tr>
	<tr>
		<td align="center">관리버튼 숨김</td>
		<td>
			<?php echo help('다음,이전,복사,이동,삭제,수정,답변,글쓰기버튼');?>
			<label><input type="checkbox" name="boset[adm_btn]" value="1"<?php echo get_checked('1', $boset['adm_btn']);?>> (관리자일경우 출력)</label>
		</td>
	</tr>
	</tbody>
	</table>
